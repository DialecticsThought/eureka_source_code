...
public class PeerAwareInstanceRegistryImpl extends AbstractInstanceRegistry implements PeerAwareInstanceRegistry {
    ...
    public boolean shouldAllowAccess(boolean remoteRegionRequired) {
        //TODO 这个是 配置文件参数peerInstancesTransferEmptyOnStartup
        if (this.peerInstancesTransferEmptyOnStartup) {
            //TODO 当前时间 > 启动时间 + 等待时间 为 true
    ...
    public void register(final InstanceInfo info, final boolean isReplication) {
        ...
        //TODO 如果外来的info 有设置leaseDuration
    ...
    public boolean renew(final String appName, final String id, final boolean isReplication) {
        //TODO 查看源码 com.netflix.eureka.registry.AbstractInstanceRegistry.renew
    ...
    /*
    * 该方法返回true 表示 client会过期
    * false表示 client 不会过期
    * */
    @Override
    public boolean isLeaseExpirationEnabled() {
        /*
        * 配置文件的属性： enable-se1f-preservation: false
        * 关闭自我保护机制，即不用保护server了，即只要有过期的client，立马清除
        * */
        ...
        * 就取决于下面的逻辑
        * getNumOfRenewsInLastMin()是得到最后一分钟续约的数量
        *  numberOfRenewsPerMinThreshold是开启client过期模式的阈值，是平均每分钟收到的续约数量。
        * 若最后一分钟收到的续约数量大于这个阈值，说明现在的client数量很多 不用考虑 可用性 的问题
        * 只要出现了过期的client直接干掉
        * 若小于这个阈值，则开启保护，为了保证可用性，出现过期client， 也不将其从注册表中干掉
         * */
     ...
     * TODO server与server之间 做replication备份 并且通过Jersey框架
     */
    private void replicateToPeers(Action action, String appName, String id,
        ...
        try {
            if (isReplication) {//TODO 客户端发来的请求 就是false
                numberOfReplicationsLastMin.increment();
            }
            // If it is a replication already, do not replicate again as this will create a poison replication
            //TODO peerEurekaNodes代表其他所有的server  peerEurekaNodes是空的 并且 还是 复制的 直接返回
            //TODO peerEurekaNodes是空的 代表就一个server
            //TODO 复制的isReplication 代表 这是从其他server发来的请求 那么不会把这个请求再一次发给其他server 仅仅是接收 防止递归
            //TODO 也就是说 server之间的通信 就是复制  client与server之间的通信是非复制的
            if (peerEurekaNodes == Collections.EMPTY_LIST || isReplication) {
                return;
            }
            //TODO 遍历所有其他的server节点
            for (final PeerEurekaNode node : peerEurekaNodes.getPeerEurekaNodes()) {
                // If the url represents this host, do not replicate to yourself.
                if (peerEurekaNodes.isThisMyUrl(node.getServiceUrl())) {
                    continue;
                }
                //TODO 把自己的信息同步到其他server 这里是心跳
    ...
    private void replicateInstanceActionsToPeers(Action action, String appName,
        ...
        try {
            ...
            switch (action) {
                ...
                //TODO 续约
                case Heartbeat:
                    //TODO 从缓存中找到 overriddenStatus
                    InstanceStatus overriddenStatus = overriddenInstanceStatusMap.get(id);
                    //TODO 根据微服务名字和实例的id找到微服务实例
                ...
                case StatusUpdate:
                    //TODO 从注册表找到微服务实例
                    infoFromRegistry = getInstanceByAppAndId(appName, id, false);
                    //TODO 某一个server通过Jersey框架向其他server（所有的region）发请求 修改状态（也就是同步数据）
                    //TODO 查看源码
