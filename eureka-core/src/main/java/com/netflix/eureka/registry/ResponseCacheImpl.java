...
public class ResponseCacheImpl implements ResponseCache {
    ...
    ResponseCacheImpl(EurekaServerConfig serverConfig, ServerCodecs serverCodecs, AbstractInstanceRegistry registry) {
        ...
        if (shouldUseReadOnlyResponseCache) {
            //TODO 有个定时器 周期性地执行任务 用来定时从readWriteCacheMap更新readOnlyCacheMap的数据
            ...
            public void run() {
                ...
                for (Key key : readOnlyCacheMap.keySet()) {
                    ...
                    try {
                        ...
                        //TODO 从读写缓存map中获取 当前遍历可以的对应的value
                        Value cacheValue = readWriteCacheMap.get(key);
                        //TODO 从 只读缓存map中获取 当前遍历可以的对应的value
                        Value currentCacheValue = readOnlyCacheMap.get(key);
                        //TODO 如果数据 不同 就将读写缓存map中的值替换掉只读缓存map中对应key 的vaLue, 实现数据更新
    ...
    public byte[] getGZIP(Key key) {
        //TODO 得到载荷（数据）
        //TODO shouldUseReadOnlyResponseCache使用只读的响应缓存 是配置文件的参数
