...
public class InstanceInfo {
     ...
     *TODO
     * unsetDirtyTimestamp 就是刚才因为脏数据标志位=true所设置的
     * lastDirtyTimestamp 是上一次脏数据标志位=true所设置的 或者叫做 当前缓存的（就是本地属性）
     */
    public synchronized void unsetIsDirty(long unsetDirtyTimestamp) {
        if (lastDirtyTimestamp <= unsetDirtyTimestamp) {//TODO 说明注册过了 客户端与服务端数据一致了
            isInstanceInfoDirty = false;
        } else {
        }
    }
